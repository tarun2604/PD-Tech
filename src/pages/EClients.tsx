...
export default function EClients() {
  ...
  useEffect(() => {
    loadClients();
    if (role === 'e.head' || role === 'e.employee') {
      loadEmployees();
    }
  }, [role, user?.id]);
  ...
  async function loadClients() {
    try {
      ...
      } else if (role === 'e.head' || role === 'admin') {
        const { data, error } = await supabase
          .from('clients')
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {filteredClients.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-gray-500">No PO received clients found</p>
        </div>
      ) : (
            ...
            client && (
              ...
              >
                ...
                <div className="flex justify-between items-center">
                  <button
                    onClick={() => navigate(`/eclients/${client.id}`)}
                    className="text-blue-600 hover:text-blue-800"
                  >
                    View Details
                  </button>
                  
                  {(role === 'e.head' || role === 'admin') && (
                    <button
                      onClick={() => {
